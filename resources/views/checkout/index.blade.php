...
<div class="bg-gray-100 min-h-screen py-10">
            ...
            <table class="w-full border border-gray-300 rounded-lg overflow-hidden">
                ...
                <tbody>
                    ...
                    <tr class="border-t bg-gray-100 font-semibold text-lg">
                        ...
                        <td class="py-3 px-4 text-blue-600">Rp{{ number_format($total, 0, ',', '.') }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <input type="text" name="name" required class="w-full border rounded px-4 py-2" value="{{ old('name', Auth::user()->name ?? '') }}">
</div>
<div>
    <label class="block mb-1 font-medium">Email</label>
    <input type="email" name="email" required class="w-full border rounded px-4 py-2" value="{{ old('email', Auth::user()->email ?? '') }}">
</div>
<div>
    <label class="block mb-1 font-medium">Telepon</label>
    <input type="text" name="phone" required class="w-full border rounded px-4 py-2" value="{{ old('phone') }}">
</div>
<div>
